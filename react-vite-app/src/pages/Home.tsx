...
export default function Home() {
  ...
  const { signInWithGoogle, authError, signingIn, user } = useAuth();
  ...
  // Handler for all sign-in buttons
  ...
  useEffect(() => {
    console.log("Auth user in Home:", user);
    if (user) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50 relative overflow-hidden">
      ...
      <header className="border-b border-white/20 bg-white/10 backdrop-blur-xl sticky top-0 z-50 shadow-lg">
        ...
        {authError && (
          <div className="text-red-600 text-center mt-2 font-semibold">{authError}</div>
        )}
      </header>
      ...
      <section className="container mx-auto px-4 py-20 text-center relative z-10">
        <div className="max-w-3xl mx-auto animate-fade-in">
          ...
          >
            {signingIn ? "Signing in..." : "Start Your First Goal"}
            <Target className="w-5 h-5 ml-2" />
          </Button>
        </div>
      </section>

      {/* Fallback button if user is authenticated but not redirected */}
      {user && (
        <div className="fixed bottom-8 right-8 z-50">
          <Button onClick={() => navigate("/dashboard")} className="bg-emerald-600 text-white rounded-full px-6 py-3 shadow-lg">
            Go to Dashboard
          </Button>
        </div>
      )}
